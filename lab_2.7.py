...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 13 12:28:30 2021
...
t1 = np.arange(-5, 10 + steps, steps)
dt = np.diff(t1)
dy = np.diff(step_ramp(t1))/dt

plt.figure(figsize =(10,6))
plt.plot(t1, step_ramp(t1), '--',label = 'step_ramp(t1)')
plt.plot(t1[range(len(dy))], dy, label = 'step_ramp(t1)/dt')
plt.legend()
plt.ylim([-2,10])
plt.grid()
